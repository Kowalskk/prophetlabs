...
const GlobalStyles = () => (
  <style>{`
    ...
    @keyframes liveTextPulse{0%{transform:scale(1);opacity:1}50%{transform:scale(1.15);opacity:0.6}100%{transform:scale(1);opacity:1}}
    @keyframes liveDotRing{0%{transform:scale(1);opacity:0.8}100%{transform:scale(2.8);opacity:0}}
    @keyframes swapSpin{0%{transform:rotate(0deg)}100%{transform:rotate(180deg)}}
    @keyframes swapSpinBack{0%{transform:rotate(180deg)}100%{transform:rotate(0deg)}}
    @keyframes pnlColReveal{from{opacity:0;max-width:0;overflow:hidden}to{opacity:1;max-width:120px}}
    @keyframes hotTipFade{from{opacity:0;transform:translateY(-4px)}to{opacity:1;transform:translateY(0)}}
    ...
    @keyframes expandContentIn{
      0%{opacity:0;transform:translateY(10px);}
      100%{opacity:1;transform:translateY(0);}
    }
    @keyframes expandContentOut{
    ...
    .pl-expand-content{
      ...
      transition:opacity 380ms ease 80ms, transform 380ms cubic-bezier(0.25,0.46,0.45,0.94) 80ms;
    }
    .pl-expand-grid.open .pl-expand-content{
      opacity:1;
    ...
    /* Class-based flash animations (stable, won't re-trigger on re-render) */
    .pl-flash-green>div:first-child{animation:rowFlashGreen 600ms ease-out!important;}
    .pl-flash-red>div:first-child{animation:rowFlashRed 600ms ease-out!important;}
    tr.pl-flash-green{animation:rowFlashGreen 600ms ease-out!important;}
    tr.pl-flash-red{animation:rowFlashRed 600ms ease-out!important;}
    ...
    .pl-table-row{transition:opacity 0.3s ease-out, transform 0.35s cubic-bezier(0.4,0,0.2,1);}
    .pl-mobile-card-item{transition:opacity 0.3s ease-out, transform 0.35s cubic-bezier(0.4,0,0.2,1);}
    ...
    /* ─── MOBILE PERF: Simplify fixed background effects ─── */
    @media(max-width:768px){
      .pl-aurora-bg>div{animation-duration:60s!important;filter:blur(80px)!important;opacity:0.5!important;}
      .pl-curtain-bg>div{animation-duration:40s!important;opacity:0.4!important;}
      .pl-depth-canvas{display:none!important;}
      .pl-watermark-bg{opacity:0.012!important;}
    }
    ...
    @media(max-width:768px){
      ...
      /* ── SIGNAL CARD ── */
      .pl-signal-card{flex-direction:column!important;text-align:left!important;gap:16px!important;padding:20px 18px!important;overflow:hidden!important;}
      .pl-signal-card>div:first-child{width:100%!important;overflow:hidden!important;}
      .pl-signal-card>div:first-child>div:last-child{flex-wrap:wrap!important;gap:6px!important;}
    ...
    /* ── SMOOTH MARKET ROW TRANSITIONS ── */
    @keyframes marketRowEnter{
      from{opacity:0;transform:translateY(8px) scale(0.98);}
      to{opacity:1;transform:translateY(0) scale(1);}
    }
    @keyframes marketRowExit{
      from{opacity:1;max-height:200px;transform:scale(1);margin-bottom:8px;}
      to{opacity:0;max-height:0;transform:scale(0.97);margin-bottom:0;overflow:hidden;}
    }
    .pl-market-row-enter{animation:marketRowEnter 0.35s cubic-bezier(0.25,0.46,0.45,0.94) both;}
    .pl-table-row-enter{animation:marketRowEnter 0.3s cubic-bezier(0.25,0.46,0.45,0.94) both;}
    ...
    /* ═══════════════════════════════════════════════════════════
       MOBILE PERFORMANCE — Eliminate GPU thrashing
       ═══════════════════════════════════════════════════════════ */
    @media(max-width:768px){
      /* CRITICAL: Remove backdrop-filter from repeated elements to prevent GPU thrashing */
      .pl-mobile-card-item>div:first-child,
      .pl-mobile-card-item *,
      .pl-metric-bar>div,
      .pl-expanded-sides>div,
      .pl-filter-grid>div,
      .pl-cat-pills>button{
      ...
      /* Keep backdrop-filter ONLY on nav and modals */
      nav{backdrop-filter:blur(80px) saturate(1.6)!important;-webkit-backdrop-filter:blur(80px) saturate(1.6)!important;}

      /* Disable will-change on mobile — saves massive GPU memory */
      .pl-mobile-cards *{will-change:auto!important;}

      /* Force hardware compositing ONLY on scroll container */
      .pl-mobile-cards{
      ...
      .pl-market-row-enter{
        animation:marketRowEnter 0.25s ease-out both!important;
      }

      /* Reduce expensive box-shadow animations on mobile */
      .pl-mobile-card-item [style*="hotPulse"],
      .pl-mobile-card-item [style*="livePulse"]{
        animation:none!important;
      }

      /* Simplify gradient text animations on mobile (expensive) */
      .pl-mobile-card-item [style*="aShift"]{
        animation-duration:8s!important;
      }

      /* Prevent text/element jitter from sub-pixel rendering */
      .pl-mobile-card-item>div:first-child{
        transform:translateZ(0);
        -webkit-transform:translateZ(0);
      }

      /* Fix overflow — nothing escapes viewport */
      .pl-dash-content{
      ...
      /* Reduce paint complexity on price flash animations */
      .pl-mobile-card-item>div:first-child{
        contain:layout style paint;
      }

      /* Disable heavy aurora/curtain animations on mobile for perf */
      [style*="willChange"]{will-change:auto!important;}

      /* Fix expanded detail overflowing */
      .pl-expanded-sides{
        max-width:100%!important;
      ...
      /* Signal card — ensure it stays within bounds */
      .pl-signal-card{
        max-width:100%!important;
        overflow:hidden!important;
      }

      /* Controls bar buttons — ensure they wrap and don't overflow */
      .pl-controls-bar>div:last-child{
        flex-wrap:wrap!important;
        max-width:100%!important;
      }
      .pl-controls-bar>div:last-child>button{
        white-space:nowrap!important;
        min-width:0!important;
        overflow:hidden!important;
      }

      /* Mobile expanded section — better organization */
      .pl-mobile-expanded-detail{
        max-width:100%!important;
        overflow-x:hidden!important;
      }

      /* Fix wager row overflow */
      .pl-wager-row{
        max-width:100%!important;
        overflow:hidden!important;
      }

      /* Order book chart — proper sizing */
      .pl-order-book-mobile svg{
        max-width:100%!important;
      }

      /* Ensure all grid children respect container */
      .pl-trade-calc-grid>div,
      .pl-metric-bar>div{
        min-width:0!important;
        overflow:hidden!important;
      }
    ...
    /* Focus visibility on table rows */
    .pl-table-row:focus-visible{outline:none!important;box-shadow:inset 2px 0 0 rgba(255,255,255,0.85),0 0 0 1px rgba(255,255,255,0.06)!important;}
    .pl-table-row:not(:focus-visible){outline:none;}
    /* Desktop table only visible >=768px */
    @media(max-width:767px){.pl-desktop-table{display:none!important;}}
    /* Mobile cards only visible <768px */
    @media(min-width:768px){.pl-mobile-cards{display:none!important;}}
...
const DepthField = () => {
  ...
  return <canvas ref={canvasRef} className="pl-depth-canvas" style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }} />;
};

const Watermark=({disabled})=>(
  <div className="pl-watermark-bg" style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",zIndex:1,pointerEvents:"none",opacity:disabled?0:0.025,userSelect:"none",transition:"opacity 0.4s ease"}}>
...
const Aurora=({disabled})=>{
  ...
  return(
  <div className="pl-aurora-bg" style={{position:"fixed",inset:0,zIndex:1,pointerEvents:"none",overflow:"hidden"}}>
...
const AuroraCurtain=({disabled})=>{
  ...
  return(
  <div className="pl-curtain-bg" style={{position:"fixed",top:0,left:0,right:0,height:"60vh",zIndex:2,pointerEvents:"none",overflow:"hidden",maskImage:"linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.6) 30%, rgba(0,0,0,0.15) 65%, transparent 100%)",WebkitMaskImage:"linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.6) 30%, rgba(0,0,0,0.15) 65%, transparent 100%)"}}>
...
const LiveDot=({secondsAgo,reconnecting,refreshPulse}={})=>(
  <span style={{display:"inline-flex",alignItems:"center",gap:7,padding:"5px 16px",borderRadius:100,background:"rgba(255,255,255,0.025)",backdropFilter:T.glassBlur,border:"none",boxShadow:"0 4px 20px rgba(0,0,0,0.3), 0 0 16px rgba(255,255,255,0.04), inset 0 1px 0 rgba(255,255,255,0.10)",animation:"livePulse 2s ease-in-out infinite"}}>
    <span style={{position:"relative",display:"inline-flex",alignItems:"center",justifyContent:"center",width:10,height:10}}>
      {refreshPulse&&<span style={{position:"absolute",inset:-1,borderRadius:"50%",border:`1.5px solid ${reconnecting?"#D4A843":"#34C759"}`,animation:"liveDotRing 600ms ease-out forwards",pointerEvents:"none"}}/>}
      <span style={{width:7,height:7,borderRadius:"50%",background:reconnecting?"#D4A843":"#34C759",boxShadow:reconnecting?"0 0 8px rgba(212,168,67,0.5)":"0 0 8px rgba(52,199,89,0.4), 0 0 14px rgba(52,199,89,0.15)",animation:reconnecting?"pulse 1s ease-in-out infinite":"none",display:"block"}}/>
    </span>
    <span className="pl-live-text" style={{fontSize:10,color:reconnecting?"#D4A843":"#34C759",fontWeight:700,letterSpacing:reconnecting?1:2,fontFamily:T.mono,animation:refreshPulse?"liveTextPulse 300ms ease-out":"none"}}>{reconnecting?"Reconnecting...":"LIVE"}</span>
...
const OrderBookChart=({market,compact=false,dualPlatform=false,platformA=null,platformB=null})=>{
  ...
  // Interpolate the Y value at a given X along a set of levels (sorted by price)
  // Returns null if px is outside the rendered X extent of the levels
  const interpY=(levels,px)=>{
    if(!levels.length)return null;
    // Find the rendered X extent of this line
    const xs=levels.map(l=>pxF(l.price));
    const xMin=Math.min(...xs),xMax=Math.max(...xs);
    // Only interpolate if cursor is within the line's X range (with small tolerance)
    if(px<xMin-4||px>xMax+4)return null;
    // find bracketing levels
    for(let i=0;i<levels.length-1;i++){
      const la=levels[i],lb=levels[i+1];
      const xa=pxF(la.price),xb=pxF(lb.price);
      const xLo=Math.min(xa,xb),xHi=Math.max(xa,xb);
      if(px>=xLo&&px<=xHi){
        const ya=qyF(la.cumulative),yb=qyF(lb.cumulative);
        const tP=Math.abs(xb-xa)<0.001?0:(px-xa)/(xb-xa);
        return ya+(yb-ya)*tP;
      }
    }
    // snap to nearest endpoint if within tolerance
    const first=levels[0],last=levels[levels.length-1];
    const xFirst=pxF(first.price),xLast=pxF(last.price);
    if(Math.abs(px-xFirst)<8)return qyF(first.cumulative);
    if(Math.abs(px-xLast)<8)return qyF(last.cumulative);
    return null;
  };
  const handleMouseMove=(e)=>{
    ...
    // Find which side (bid or ask) we're closer to for tooltip
    const allLevels=[...bidLevels,...askLevels].sort((a,b)=>Math.abs(a.price-price)-Math.abs(b.price-price));
    const closest=allLevels[0];
    // Interpolate Y for platform A (bid or ask, whichever is closer)
    const yABid=interpY(bidLevels,x);
    const yAsk=interpY(askLevels,x);
    // pick whichever line is closer to cursor Y (we don't have cursor Y in SVG coords easily, use closest price side)
    const onBidSide=price<=midPrice;
    const yA=onBidSide?(yABid!==null?yABid:yAsk):(yAsk!==null?yAsk:yABid);
    // Platform B
    const yBBid=dualPlatform?interpY(opinBidLevels,x):null;
    const yBAsk=dualPlatform?interpY(opinAskLevels,x):null;
    const yB=dualPlatform?(onBidSide?(yBBid!==null?yBBid:yBAsk):(yBAsk!==null?yBAsk:yBBid)):null;
    if(closest)setHover({x,price:closest.price,depth:closest.cumulative,yA,yB});
  };
    ...
    <div style={{position:"relative",borderRadius:16,overflow:"hidden",background:"radial-gradient(ellipse at 50% 0%, rgba(255,255,255,0.022) 0%, rgba(0,0,0,0) 70%), rgba(6,6,10,0.95)",boxShadow:"inset 0 0 0 0.5px rgba(255,255,255,0.07), 0 24px 80px rgba(0,0,0,0.6)"}}>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${cW} ${cH}`} preserveAspectRatio="none"
        ...
        {hover&&<>
          ...
          {/* Platform A crosshair dot — smoothly interpolated */}
          {hover.yA!==null&&<>
            <circle cx={hover.x} cy={hover.yA} r="6"
              fill="rgba(0,0,0,0.5)" stroke={cA.color} strokeWidth="1.5" pointerEvents="none" opacity="0.9"/>
            <circle cx={hover.x} cy={hover.yA} r="2.5"
              fill={cA.color} pointerEvents="none"/>
          </>}
          {/* Platform B crosshair dot — show both */}
          {dualPlatform&&hover.yB!==null&&<>
            <circle cx={hover.x} cy={hover.yB} r="6"
              fill="rgba(0,0,0,0.5)" stroke={cB.color} strokeWidth="1.5" pointerEvents="none" opacity="0.9"/>
            <circle cx={hover.x} cy={hover.yB} r="2.5"
              fill={cB.color} pointerEvents="none"/>
          </>}
          {/* Tooltip panel */}
          {tooltipEls&&<>
...
const Dash=({onNavigate:nav,effectsDisabled,toggleEffects,onLegalOpen})=>{
  ...
  const expandRow=(id)=>{
    setExpanded(id);
    // Double rAF: first frame paints the tr into DOM at grid-template-rows:0fr,
    // second frame adds .open so the CSS transition fires from 0fr→1fr
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      setOpenRows(prev=>{const n=new Set(prev);n.add(id);return n;});
    }));
  };
  const collapseRow=(id)=>{
    // Remove .open — CSS transition plays 0fr collapse + content fades out
    setOpenRows(prev=>{const n=new Set(prev);n.delete(id);return n;});
    setClosingRows(prev=>{const n=new Set(prev);n.add(id);return n;});
    setTimeout(()=>{
      setExpanded(null);
      setClosingRows(prev=>{const n=new Set(prev);n.delete(id);return n;});
    },EXPAND_MS);
  };
  ...
  const[newMarketIds,setNewMarketIds]=useState(new Set());
  ...
  useEffect(()=>{
    const handler=(e)=>{
      if(e.key==="Escape"){
        ...
        if(expanded!==null){collapseRow(expanded);setSel(null);}
      }
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{
      ...
      setRefreshPulse(true);setTimeout(()=>setRefreshPulse(false),700);
      setPrices(prev=>{
  ...
  // Stable display order — freeze while a row is expanded so the list
  // never jumps while the user is interacting with an expanded market.
  const stableOrderRef=React.useRef([]);
  const displayFiltered=useMemo(()=>{
    if(expanded){
      // Re-use previous order, splice in any brand-new IDs at end
      const prev=stableOrderRef.current;
      const filteredSet=new Set(filtered.map(o=>o.id));
      // keep markets that are still in filtered list, in their stable order
      const stable=prev.filter(o=>filteredSet.has(o.id));
      // append any new markets that weren't in the stable list
      const stableIds=new Set(stable.map(o=>o.id));
      filtered.forEach(o=>{if(!stableIds.has(o.id))stable.push(o);});
      return stable;
    }
    // Not expanded — use live sorted order and update ref
    stableOrderRef.current=filtered;
    return filtered;
  },[filtered, expanded]);

  const best=filtered.length>0?filtered[0]:null;

  // Top-3 APR market IDs for dynamic HOT badge
  const top3AprIds=useMemo(()=>{const sorted=[...prices].sort((a,b)=>dynAPR(b)-dynAPR(a));return new Set(sorted.slice(0,3).map(o=>o.id));},[prices,mktA,mktB]);

  // Track newly appearing markets for smooth entry animation
  useEffect(()=>{
    const currentIds=new Set(filtered.map(o=>o.id));
    const prevIds=prevFilteredIdsRef.current;
    const entering=new Set();
    currentIds.forEach(id=>{if(!prevIds.has(id))entering.add(id);});
    prevFilteredIdsRef.current=currentIds;
    if(entering.size>0){
      setNewMarketIds(entering);
      const timer=setTimeout(()=>setNewMarketIds(new Set()),400);
  ...
  return(
    <div style={{background:"transparent",minHeight:"100vh",position:"relative",zIndex:10}}>
      <nav style={{background:"rgba(255,255,255,0.028)",backdropFilter:"blur(120px) saturate(1.4)",WebkitBackdropFilter:"blur(120px) saturate(1.4)",borderBottom:"none",boxShadow:"0 8px 40px rgba(0,0,0,0.50), inset 0 -1px 0 rgba(255,255,255,0.07), 0 0 0 0.5px rgba(255,255,255,0.05)",position:"sticky",top:0,zIndex:50}}>
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 24px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div className="pl-nav-right" style={{display:"flex",alignItems:"center",gap:16}}>
            <LiveDot secondsAgo={30-countdown} reconnecting={countdown<=0} refreshPulse={refreshPulse}/>
      ...
      <div className="pl-dash-content" style={{maxWidth:1400,margin:"0 auto",padding:"28px 24px",position:"relative",zIndex:10,overflowX:"hidden"}}>
        ...
        <div className="pl-controls-bar" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:12}}>
          ...
          <div className="pl-compare-label" style={{display:"inline-flex",alignItems:"center",gap:4,padding:"4px 8px 4px 10px",borderRadius:12,background:"rgba(255,255,255,0.028)",backdropFilter:"blur(110px) saturate(1.35)",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.08), 0 2px 8px rgba(0,0,0,0.2)"}}>
            ...
            <button onClick={e=>{e.currentTarget.style.animation="swapSpin 300ms ease-out forwards";setTimeout(()=>{if(e.currentTarget){e.currentTarget.style.animation="";}},320);const tmpA=mktA;setMktA(mktB);setMktB(tmpA);}} title="Swap platforms" style={{width:22,height:22,borderRadius:6,border:"none",cursor:"pointer",background:"rgba(255,255,255,0.025)",color:T.textSecondary,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",transition:"color .2s"}}
              onMouseOver={e=>{e.currentTarget.style.color=T.text;e.currentTarget.style.background="rgba(255,255,255,0.055)";}}
              onMouseOut={e=>{e.currentTarget.style.color=T.textSecondary;e.currentTarget.style.background="rgba(255,255,255,0.025)";}}>⇄</button>
          ...
          {/* Global wager input in controls bar */}
          <div style={{display:"inline-flex",alignItems:"center",gap:0,
            background:"rgba(255,255,255,0.038)",
            backdropFilter:"blur(110px) saturate(1.35)",
            WebkitBackdropFilter:"blur(110px) saturate(1.35)",
            borderRadius:14,overflow:"hidden",
            boxShadow:"inset 0 1px 0 rgba(255,255,255,0.14), 0 2px 10px rgba(0,0,0,0.3)",
            border:"none",
          }}>
            <button onClick={e=>{e.stopPropagation();setWager(v=>Math.max(10,v-100));}}
              title="Decrease wager"
              style={{padding:"11px 12px",background:"transparent",border:"none",cursor:"pointer",
                color:"rgba(255,255,255,0.35)",fontFamily:T.mono,fontSize:14,fontWeight:300,lineHeight:1,
            ...
            >−</button>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"6px 4px",cursor:"text",minWidth:68}}
              onClick={e=>e.stopPropagation()}>
              <input type="number" min={10} value={wager}
                onChange={e=>{const v=Math.max(10,Number(e.target.value)||10);setWager(v);}}
                onClick={e=>e.stopPropagation()}
                title="Global wager amount (feeds profit calculations)"
                style={{width:64,background:"transparent",border:"none",outline:"none",
                  fontFamily:T.mono,fontSize:12,fontWeight:700,color:"rgba(255,255,255,0.9)",
                  textAlign:"center",padding:0,MozAppearance:"textfield"}}/>
              <span style={{fontSize:8,fontFamily:T.mono,color:"rgba(255,255,255,0.3)",letterSpacing:1.5,fontWeight:600,marginTop:1}}>WAGER</span>
            </div>
            <button onClick={e=>{e.stopPropagation();setWager(v=>v+100);}}
        ...
        <Card hover={false} style={{overflow:"visible",borderRadius:18}} className="pl-desktop-table">
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {displayFiltered.map((o,idx)=>{
                  const isClosing=closingRows.has(o.id);
                  const isExp=resolvedExpanded===o.id||isClosing;
                  const _localPair=(resolvedExpanded===o.id)?(rowPairs[o.id]||null):null;
                  const _lmktA=_localPair?_localPair.a:mktA;
                  const _lmktB=_localPair?_localPair.b:mktB;
                  const _lpA=PLATFORMS[_lmktA],_lpB=PLATFORMS[_lmktB];
                  const _lpAColor=_lpA.color,_lpBColor=_lpB.color,_lpAIcon=_lpA.icon,_lpBIcon=_lpB.icon,_lpAName=_lpA.name,_lpBName=_lpB.name;
                  const _lpAFee=_lpA.fee,_lpBFee=_lpB.fee;
                  const _lGetPA=(o)=>o.allPrices?.[_lmktA]||o.prices?.[_lmktA]||{yes:0.5,no:0.5};
                  const _lGetPB=(o)=>o.allPrices?.[_lmktB]||o.prices?.[_lmktB]||{yes:0.5,no:0.5};
                  const td=isExp?tradeDetail(o,wager):null;
                  // Check if this is the first non-profitable row when profitableOnly is false
                  const prevO=idx>0?displayFiltered[idx-1]:null;
                  ...
                  // Local pair override vars for expanded row
                  const _lPolyIsYes=_lGetPA(o).yes<_lGetPB(o).yes;
                  const _lYesPlatform=_lPolyIsYes?_lpAName:_lpBName;
                  const _lNoPlatform=_lPolyIsYes?_lpBName:_lpAName;
                  const _lYesColor=_lPolyIsYes?_lpAColor:_lpBColor;
                  const _lNoColor=_lPolyIsYes?_lpBColor:_lpAColor;
                  ...
                  <tr className={"pl-table-row"+(newMarketIds.has(o.id)?" pl-table-row-enter":"")+(flashRows[o.id]==="green"?" pl-flash-green":"")+(flashRows[o.id]==="red"?" pl-flash-red":"")} onClick={()=>{if(isExp){collapseRow(o.id);}else{setSel(o);expandRow(o.id);setTimeout(()=>{rowRefs.current.get(o.id)?.scrollIntoView({behavior:'smooth',block:'nearest'});},50);}}}
                    ...
                    onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();setSel(o);if(isExp)collapseRow(o.id);else expandRow(o.id);}}}
                    style={{borderBottom:"none",cursor:"pointer",transition:"background 0.2s ease, box-shadow 0.2s ease, opacity 0.2s ease",background:isExp?"rgba(255,255,255,0.03)":sel?.id===o.id?"rgba(255,255,255,0.02)":"transparent",boxShadow:isExp?"inset 3px 0 0 "+T.auroraText:"none",opacity:o.apr<=0?0.6:1}}
                    ...
                    <td style={{padding:"14px 24px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                        {(signalScore(o)>=70&&top3AprIds.has(o.id))&&<span title="High signal score + top APR opportunity." style={{display:"inline-flex",alignItems:"center",gap:3,padding:"2px 8px",borderRadius:8,background:"rgba(255,255,255,0.022)",backdropFilter:"blur(100px) saturate(1.30)",border:"none",animation:"hotPulse 2s ease-in-out infinite",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.10)",fontSize:9,fontWeight:700,letterSpacing:1.5,fontFamily:T.mono,color:"#ffffff",cursor:"help"}}><IF/> HOT</span>}
                  ...
                  <tr><td colSpan={matrixView?12:13} style={{padding:0,borderBottom:"none",background:T.auroraSoft}}>
                    <div className={`pl-expand-grid${openRows.has(o.id)&&!isClosing?" open":""}`}>
                      <div className="pl-expand-inner">
                        <div className="pl-expand-content">
                    <div style={{padding:"24px 28px 28px"}}>
                      {/* Compact inline platform pair selector */}
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
                        <span style={{fontSize:8,fontWeight:700,letterSpacing:2,fontFamily:T.mono,color:T.textTertiary}}>COMPARING</span>
                        {PLATFORM_KEYS.map(k=>{const lp=rowPairs[o.id];const localA=lp?.a||mktA;const localB=lp?.b||mktB;const isA=localA===k;const isB=localB===k;const pl=PLATFORMS[k];return(
                          <button key={k} onClick={e=>{e.stopPropagation();const lp2=rowPairs[o.id]||{a:mktA,b:mktB};let na=lp2.a,nb=lp2.b;if(isA){if(PLATFORM_KEYS.filter(x=>x!==nb).length>0){na=PLATFORM_KEYS.filter(x=>x!==nb&&x!==k)[0]||k;}}else if(isB){nb=PLATFORM_KEYS.filter(x=>x!==na&&x!==k)[0]||k;}else{nb=k;}setRowPairs(prev=>({...prev,[o.id]:{a:na,b:nb}}));}}
                            style={{padding:"3px 10px",borderRadius:100,border:"none",cursor:"pointer",fontFamily:T.mono,fontSize:9,fontWeight:700,letterSpacing:0.5,transition:"all .15s",
                              background:isA?`${pl.color}22`:isB?`${pl.color}14`:"rgba(255,255,255,0.025)",
                              color:isA||isB?pl.color:T.textTertiary,
                              boxShadow:isA?`inset 0 1px 0 rgba(255,255,255,0.12), 0 0 10px ${pl.color}22`:isB?"inset 0 1px 0 rgba(255,255,255,0.08)":"inset 0 1px 0 rgba(255,255,255,0.06)",
                              border:isA?`1px solid ${pl.color}44`:isB?`1px solid ${pl.color}22`:"1px solid transparent",
                            }}>{pl.icon} {pl.short}{isA&&<span style={{marginLeft:4,fontSize:7,opacity:0.7}}>A</span>}{isB&&<span style={{marginLeft:4,fontSize:7,opacity:0.7}}>B</span>}</button>
                        );})}
                        {rowPairs[o.id]&&<button onClick={e=>{e.stopPropagation();setRowPairs(prev=>{const n={...prev};delete n[o.id];return n;});}} style={{padding:"2px 8px",borderRadius:100,border:"none",cursor:"pointer",background:"rgba(255,255,255,0.02)",color:T.textTertiary,fontFamily:T.mono,fontSize:8,letterSpacing:0.5,transition:"all .15s"}} onMouseOver={e=>e.currentTarget.style.color=T.text} onMouseOut={e=>e.currentTarget.style.color=T.textTertiary}>↩ global</button>}
                      </div>
                      {/* Tab bar */}
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,padding:"8px 14px",borderRadius:12,background:"rgba(255,255,255,0.015)",backdropFilter:"blur(100px) saturate(1.30)",border:"none",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.07)"}}>
                        ...
                        <span style={{fontSize:10,color:T.textSecondary,fontFamily:T.body,lineHeight:1.5}}>
                          <span style={{fontWeight:600,color:T.text}}>Strategy:</span> Buy <span style={{color:_lYesColor,fontWeight:600}}>YES</span> on <span style={{color:_lYesColor,fontWeight:600}}>{_lYesPlatform}</span>, <span style={{color:_lNoColor,fontWeight:600}}>NO</span> on <span style={{color:_lNoColor,fontWeight:600}}>{_lNoPlatform}</span>. If the event resolves either way, one side pays out $1 per share — locking in the spread as profit.
                        </span>
                      </div>
                      ...
                      <div style={{gridArea:"1/1",opacity:activeTab==="ORDER BOOK"?1:0,transition:"opacity 0.22s ease",pointerEvents:activeTab==="ORDER BOOK"?"auto":"none",willChange:"opacity"}}>
                        <div style={{background:"rgba(255,255,255,0.012)",borderRadius:18,border:"none",overflow:"visible",boxShadow:"0 8px 40px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.07)",padding:"20px 24px"}}>
                          ...
                          <OrderBookChart market={o} dualPlatform platformA={_lpA} platformB={_lpB}/>
                      ...
                      <div className="pl-expanded-actions" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginTop:20}}>
                        ...
                        <button style={{padding:"12px",borderRadius:14,border:"none",cursor:"pointer",fontFamily:T.mono,fontWeight:700,fontSize:11,letterSpacing:2,background:"rgba(255,255,255,0.025)",backdropFilter:"blur(110px) saturate(1.35)",color:T.textSecondary,display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"all .2s",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.10), 0 4px 16px rgba(0,0,0,0.25)"}}
                          ...
                          onClick={e=>{e.stopPropagation();const prA=getPriceA(o),prB=getPriceB(o);const txt=`ProphetLabs Signal — ${o.event}\nBuy YES on ${pAName} @ ${(prA.yes*100).toFixed(1)}¢ | Buy NO on ${pBName} @ ${(prB.no*100).toFixed(1)}¢\nWager: $${wager.toLocaleString()} | Net PnL: $${td.netPnl.toFixed(2)} | APR: ${dynAPR(o).toFixed(1)}% | Expiry: ${o.expiry}\nprophetlabs.io`;navigator.clipboard.writeText(txt).then(()=>{showToast("Copied!","");});}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> COPY SETUP</button>
                      </div>
                    </div>{/* end padding div */}
                    </div>{/* end pl-expand-content */}
                        </div>{/* end pl-expand-inner */}
                      </div>{/* end pl-expand-grid */}
                  </td></tr>
                  )}
                  </React.Fragment>
                );})}
          ...
          {filtered.length===0&&<div style={{textAlign:"center",padding:"72px 24px",color:T.textTertiary,animation:"revealFade 0.3s ease-out"}}>
            <div style={{marginBottom:20,display:"flex",justifyContent:"center"}}>
              {catFilter==="Saved"?<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke={T.textTertiary} strokeWidth="1.2" style={{opacity:0.4}}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
              :<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke={T.textTertiary} strokeWidth="1.1" style={{opacity:0.45}}>
                <path d="M2 12 Q6 6 12 9 Q18 12 22 6" strokeLinecap="round"/>
                <path d="M2 18 Q7 11 13 15 Q18 18 22 12" strokeLinecap="round" opacity="0.6"/>
                <circle cx="12" cy="12" r="1.5" fill={T.textTertiary} opacity="0.6"/>
              </svg>}
            </div>
            <div style={{fontSize:16,fontWeight:500,color:T.textSecondary,marginBottom:8,fontFamily:T.display}}>{catFilter==="Saved"&&savedMarkets.size===0?"No saved markets yet":"No signals match your filters"}</div>
            <div style={{fontSize:12,color:T.textTertiary,fontFamily:T.body,maxWidth:340,margin:"0 auto",lineHeight:1.8,marginBottom:24}}>{catFilter==="Saved"&&savedMarkets.size===0?<span style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
            ...
            <button onClick={resetAll} style={{padding:"10px 28px",borderRadius:100,border:"none",cursor:"pointer",fontFamily:T.mono,fontWeight:600,fontSize:11,letterSpacing:1.5,background:"rgba(255,255,255,0.04)",backdropFilter:"blur(110px) saturate(1.35)",WebkitBackdropFilter:"blur(110px) saturate(1.35)",color:T.text,boxShadow:"inset 0 1px 0 rgba(255,255,255,0.14), 0 4px 20px rgba(0,0,0,0.35), 0 0 0 0.5px rgba(255,255,255,0.08)",transition:"all .2s"}} onMouseOver={e=>{e.currentTarget.style.background="rgba(255,255,255,0.08)";e.currentTarget.style.boxShadow="inset 0 1px 0 rgba(255,255,255,0.20), 0 6px 28px rgba(0,0,0,0.4), 0 0 0 0.5px rgba(255,255,255,0.12)";}} onMouseOut={e=>{e.currentTarget.style.background="rgba(255,255,255,0.04)";e.currentTarget.style.boxShadow="inset 0 1px 0 rgba(255,255,255,0.14), 0 4px 20px rgba(0,0,0,0.35), 0 0 0 0.5px rgba(255,255,255,0.08)";}}>↺ Reset filters</button>
          </div>}
        </Card>
        ...
        <div className="pl-mobile-cards">
          ...
          {displayFiltered.map((o,idx)=>{
            ...
            return(
            <div key={o.id} className={"pl-mobile-card-item"+(newMarketIds.has(o.id)?" pl-market-row-enter":"")+(flashRows[o.id]==="green"?" pl-flash-green":flashRows[o.id]==="red"?" pl-flash-red":"")} style={{marginBottom:8}}>
              <div onClick={()=>{if(isExp)collapseRow(o.id);else{setSel(o);expandRow(o.id);}}}
                style={{background:isExp?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.012)",borderRadius:14,padding:"14px 16px",cursor:"pointer",transition:"background 0.2s ease, box-shadow 0.2s ease",border:"none",boxShadow:isExp?"inset 3px 0 0 "+T.auroraText+", "+T.cardShadow:T.glassShadow}}>                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
                      ...
                      {(signalScore(o)>=70&&top3AprIds.has(o.id))&&<span title="High signal score + top APR opportunity." style={{display:"inline-flex",alignItems:"center",gap:3,padding:"2px 6px",borderRadius:6,background:"rgba(255,255,255,0.022)",animation:"hotPulse 2s ease-in-out infinite",fontSize:8,fontWeight:700,letterSpacing:1,fontFamily:T.mono,color:"#ffffff",cursor:"help"}}><IF/> HOT</span>}
                    </div>
              ...
              {isExp&&td&&(
              <div className="pl-mobile-expanded-detail" style={{background:T.auroraSoft,borderRadius:"0 0 16px 16px",padding:"14px",marginTop:-4,overflow:"hidden",animation:"revealFade 0.25s ease-out"}}>
                {/* Tab bar */}
                ...
                <div style={{marginTop:10}}>
                  <div style={{padding:"12px 14px",borderRadius:12,background:"rgba(255,255,255,0.02)",boxShadow:"0 4px 24px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.09)"}}>
