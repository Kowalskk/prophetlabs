  ...
  return (
    <div style={{
      ...
      <div style={{position:"relative",zIndex:1,textAlign:"center"}}>
        ...
        <div style={{fontFamily:T.mono,fontSize:56,fontWeight:300,letterSpacing:"-2px",marginBottom:14,
    ...
    <Card hover={false} style={{padding:0,marginBottom:20,overflow:"hidden"}}>
      ...
      <div style={{padding:"14px 24px 20px"}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {channels.map(ch=>(
              <button key={ch} onClick={()=>setCfg(p=>({...p,channel:ch,saved:false}))}
                style={{flex:1,padding:"10px 14px",borderRadius:12,border:"none",cursor:"pointer",fontFamily:T.mono,fontSize:11,fontWeight:600,letterSpacing:0.5,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={()=>{setCfg(p=>{const next={...p,saved:true};return next;});}}            style={{padding:"10px 24px",borderRadius:12,border:"none",cursor:"pointer",fontFamily:T.mono,fontWeight:700,fontSize:11,letterSpacing:1.5,
...
const Dash=({onNavigate:nav,effectsDisabled,toggleEffects})=>{
  ...
  const wsStatusRef   = useRef({kalshi:"closed",predict:"closed"});
  const [wsStatus,setWsStatus] = useState({kalshi:"closed",predict:"closed"});
  ...
  const teardownWs = useCallback(()=>{
    ...
    clearInterval(predictHeartbeatRef.current);
    [wsKalshiRef,wsPredictRef].forEach(ref=>{
      if(ref.current&&ref.current.readyState!==WebSocket.CLOSED){
        ref.current.close();
      }
      ref.current=null;
    });
    setWsStatus({kalshi:"closed",predict:"closed"});
  },[]);
  ...
  },[dataMode,apiKeys.predict,connectKalshiWs,connectPredictWs,teardownWs]);
        ...
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 24px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <button
              ...
              style={{display:"inline-flex",alignItems:"center",gap:5,height:28,borderRadius:8,border:"none",cursor:"pointer",padding:"0 10px",
            ...
            <button
              ...
              style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:28,height:28,borderRadius:8,border:"none",cursor:"pointer",
            ...
            <button aria-label={matrixView?"Switch to pair view":"Switch to matrix view"} onClick={()=>setMatrixView(v=>!v)} title={matrixView?"Pair view":"Matrix view"} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",gap:4,height:28,borderRadius:8,border:"none",cursor:"pointer",padding:"0 10px",
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:24,flexWrap:"wrap"}}>
            ...
            <div style={{flex:"0 0 auto"}}>
              ...
              <div style={{display:"flex",gap:4}}>
                {["mock","live"].map(m=>(
                  <button key={m} onClick={()=>setDataMode(m)} style={{padding:"6px 14px",borderRadius:8,border:"none",cursor:"pointer",fontFamily:T.mono,fontSize:9,fontWeight:700,letterSpacing:1.5,transition:"all .15s",
      ...
      {dataMode==="live"&&<div style={{background:"rgba(255,255,255,0.012)",borderBottom:"1px solid rgba(255,255,255,0.04)",padding:"6px 24px"}}>
          ...
          {[
            ...
            return(
              <div key={label} style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                {!needsKey&&<span style={{fontSize:8,fontFamily:T.mono,
        ...
        <div className="pl-pair-selector" style={{marginBottom:20,padding:"18px 24px",borderRadius:18,background:"rgba(255,255,255,0.035)",backdropFilter:"blur(120px) saturate(1.4)",WebkitBackdropFilter:"blur(120px) saturate(1.4)",boxShadow:T.glassShadow,position:"relative",zIndex:30,overflow:"visible"}}>
          ...
          <div style={{textAlign:"center",marginTop:12,paddingTop:12,borderTop:"1px solid rgba(255,255,255,0.04)",display:"flex",alignItems:"center",justifyContent:"center",gap:12,flexWrap:"wrap"}}>
            ...
            {matchedCount>0&&<button onClick={()=>setMatchedOnly(p=>!p)} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:8,border:"none",cursor:"pointer",fontFamily:T.mono,fontSize:8,fontWeight:700,letterSpacing:1.2,transition:"all .2s",
        ...
        <div style={{display:"flex",gap:6,marginBottom:6,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button onClick={()=>setProfitableOnly(p=>!p)}
            style={{padding:"5px 14px",borderRadius:100,border:profitableOnly?"1px solid rgba(52,199,89,0.3)":"1px solid rgba(255,255,255,0.08)",cursor:"pointer",fontFamily:T.mono,fontSize:9,fontWeight:700,letterSpacing:1,textTransform:"uppercase",transition:"all .2s",
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {filtered.map((o,idx)=>{
                  ...
                  <tr className="pl-table-row" onClick={()=>{const newExp=isExp?null:o.id;setSel(o);setExpanded(newExp);if(newExp!==null){setTimeout(()=>{rowRefs.current.get(o.id)?.scrollIntoView({behavior:'smooth',block:'nearest'});},50);// Fetch real orderbooks when row expands (live mode + has sources)
if(dataMode==="live"&&o._sources&&!realOrderbooks[o.id]){fetchRealOrderbooks(o,apiKeys).then(books=>{if(Object.keys(books).length>0)setRealOrderbooks(prev=>({...prev,[o.id]:books}));}).catch(()=>{});}}}}
                    ...
                    <td style={{padding:"14px 24px"}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,flexWrap:"wrap"}}>
                        ...
                        {o.matchScore>0&&<span title={`Cross-platform match confidence: ${Math.round(o.matchScore*100)}%`} style={{display:"inline-flex",alignItems:"center",gap:3,padding:"2px 7px",borderRadius:6,
            ...
            <div key={o.id} className="pl-mobile-card-item" style={{marginBottom:8,animationDelay:`${idx*0.04}s`}}>
              ...
              <div style={{background:T.auroraSoft,borderRadius:"0 0 14px 14px",padding:"16px",marginTop:-4}}>
                ...
                <div style={{display:"flex",gap:0,marginBottom:12,borderRadius:10,overflow:"hidden",background:"rgba(255,255,255,0.012)",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.06)"}}>
                  {["TRADE","ORDER BOOK"].map(tab=>(
                    <button key={tab} onClick={e=>{e.stopPropagation();setTab(tab);}}
                      style={{flex:1,padding:"8px 10px",border:"none",cursor:"pointer",fontFamily:T.mono,fontSize:8,fontWeight:700,letterSpacing:1.5,
                        background:activeTab===tab?"rgba(255,255,255,0.04)":"transparent",
                        color:activeTab===tab?T.text:T.textTertiary,
                        boxShadow:activeTab===tab?"inset 0 -2px 0 rgba(255,255,255,0.3)":"none",
                        transition:"all .2s"}}>{tab}</button>
